...
class DeviceActionsController:
    ...
    def show_context_menu(
        self, global_position: QPoint, device_serial: str, checkbox_widget
    ) -> None:
        ...
        select_only_action.triggered.connect(
            lambda: self.select_only_device(device_serial)
        )
        ...
        view_logcat_action.triggered.connect(
            lambda: self.window.view_logcat_for_device(device_serial)
        )
        ...
        ui_inspector_action.triggered.connect(
            lambda: self.launch_ui_inspector_for_device(device_serial)
        )
        context_menu.addSeparator()

        reboot_action = context_menu.addAction("🔄 Reboot Device")
        reboot_action.triggered.connect(
            lambda: self.reboot_single_device(device_serial)
        )

        screenshot_action = context_menu.addAction("📷 Take Screenshot")
        screenshot_action.triggered.connect(
            lambda: self.take_screenshot_single_device(device_serial)
        )

        scrcpy_action = context_menu.addAction("🖥️ Mirror Device (scrcpy)")
        scrcpy_action.triggered.connect(
            lambda: self.launch_scrcpy_single_device(device_serial)
        )
        ...
        copy_info_action.triggered.connect(
            lambda: self.copy_single_device_info(device_serial)
        )
    ...
    def deselect_device(self, target_serial: str) -> None:
        current = [
            serial
            for serial in self.window.device_selection_manager.get_selected_serials()
    ...
    def copy_single_device_info(self, device_serial: str) -> None:
        ...
        try:
            ...
            self.window.show_info(
                "📋 Copied!",
                f"Device information copied to clipboard for:\n{device.device_model}",
            )
    ...
    def copy_selected_device_info(self) -> None:
        ...
        for index, device in enumerate(checked_devices, start=1):
            ...
            try:
                additional_info = (
                    self.window.device_list_controller.get_additional_device_info(
                ...
                section.extend(
                    [
                        ...
                        format_detail(
                            "Screen Size", additional_info.get("screen_size")
                        ),
                        format_detail(
                            "Screen Density", additional_info.get("screen_density")
                        ),
                        format_detail(
                            "CPU Architecture", additional_info.get("cpu_arch")
                        ),
                        "",
                        "BATTERY INFORMATION:",
                        format_detail(
                            "Battery Level", additional_info.get("battery_level")
                        ),
                        format_detail(
                            "Capacity (mAh)",
                            additional_info.get("battery_capacity_mah"),
                        ),
                        format_detail(
                            "Battery mAs", additional_info.get("battery_mas")
                        ),
                        format_detail(
                            "Estimated DOU", additional_info.get("battery_dou_hours")
                        ),
                    ]
                )
            except Exception as exc:
                ...
                logger.warning(
                    "Could not retrieve hardware info for %s: %s",
                    device.device_serial_num,
                    exc,
                )
        ...
        logger.info(
            "Copied comprehensive information for %s devices", len(checked_devices)
        )
    ...
    def _restore_device_selections(
        self, selections: Dict[str, bool], active_serial: Optional[str]
    ) -> None:
        selected_serials = [
            serial for serial, is_checked in selections.items() if is_checked
        ]
