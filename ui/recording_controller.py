...
class RecordingController(QObject):
    ...
    def start_screen_record(self) -> None:
        ...
        def recording_callback(device_name, device_serial, duration, filename, output_path):
            self.window.recording_stopped_signal.emit(device_name, device_serial, duration, filename, output_path)
    ...
    def stop_screen_record(self) -> None:
        ...
        if selected_devices:
            ...
            if not devices_to_stop:
                statuses = getattr(self._recording_manager, 'get_all_recording_statuses', lambda: {})()
                ...
                self.window.error_handler.show_warning('No Selected Devices Recording', details)
                return
    ...
    def _on_stop_screen_record_task_completed(
        ...
        for serial in stopped:
            device_info = self.window.device_dict.get(serial) if hasattr(self.window, 'device_dict') else None
    ...
    def handle_recording_stopped(self, device_name, device_serial, duration, filename, output_path):
        ...
        record = self.window.device_recordings.setdefault(device_serial, {
            'segments': [],
            'elapsed_before_current': 0.0,
            'ongoing_start': None,
            'display_seconds': 0,
    ...
    def _handle_segment_completed(self, event: RecordingProgressEvent) -> None:
        ...
        segment_label = (
            f"{event.segment_index:02d}" if isinstance(event.segment_index, int) else "?"
        )
        self.window.write_to_console(
